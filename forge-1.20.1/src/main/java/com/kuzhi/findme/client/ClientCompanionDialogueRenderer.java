...
final class ClientCompanionDialogueRenderer {
    ...
    static void renderGui(RenderGuiEvent.Post event) {
        ...
        int callAlpha = Math.round(ClientCompanionDialogueState.alpha(event.getPartialTick()) * 255.0f);
        if (callAlpha <= 0) {
            return;
        }
        ...
        int replyAlpha = Math.round(ClientCompanionDialogueState.replyAlpha(event.getPartialTick()) * 255.0f);
