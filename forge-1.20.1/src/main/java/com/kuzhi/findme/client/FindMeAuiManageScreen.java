...
import com.kuzhi.findme.api.client.FindMeExternalCompanionEditor;
...
public final class FindMeAuiManageScreen extends FindMeAuiOverlayScreen {
    ...
    private static FindMeExternalCompanionEditor externalEditor;
    ...
    private int contextExternalSlotIndex = -1;
    ...
    private final CompanionTeamSelectionDraft warehouseSelection =
            new CompanionTeamSelectionDraft(FindMeUiSettings.TEAM_CAPACITY);
    private WarehouseMenuPage warehouseMenuPage = WarehouseMenuPage.MAIN;
    private TeamMenuPage teamMenuPage = TeamMenuPage.MAIN;
    private BackupMenuPage backupMenuPage = BackupMenuPage.MAIN;
    ...
    private int scrollRestoreTicks;
    private int doctorLayoutCommitTicks;
    private double pendingTeamScrollRestore = Double.NaN;
    private double pendingWarehouseScrollRestore = Double.NaN;
    ...
    private BackupFilter backupFilter = BackupFilter.AUTOMATIC;
    ...
    private int cardToggleSlotIndex = -1;
    ...
    public static void openExternalEditor(FindMeExternalCompanionEditor editor) {
        if (editor == null || !ClientFindMeModuleState.enabled(FindMeModule.MANAGEMENT)) return;
        if (cachedScreen == null) cachedScreen = new FindMeAuiManageScreen();
        cachedScreen.prepareForOpen();
        externalEditor = editor;
        cachedScreen.category = Category.COMPANION;
        cachedScreen.selectedTeam = Math.max(0,
                ClientCompanionTeamState.currentTeam(editor.teamTarget()));
        current = cachedScreen;
        MinecraftAccess.setScreen(cachedScreen);
    }

    public static boolean externalEditorActive() {
        return externalEditor != null;
    }

    public static void refreshExternalEditor() {
        if (current == null || externalEditor == null) return;
        current.refreshDocument();
        if (current.contextOpen) current.refreshContextOverlay();
    }
    ...
    private void prepareForOpen() {
        ...
        backupFilter = BackupFilter.AUTOMATIC;
        ...
        scrollRestoreTicks = 0;
        doctorLayoutCommitTicks = 0;
        pendingTeamScrollRestore = Double.NaN;
        pendingWarehouseScrollRestore = Double.NaN;
        pendingSettingsScrollRestore = Double.NaN;
        pendingDoctorBackupScrollRestore = Double.NaN;
        search = "";
        status = "";
        statusTicks = 0;
        contextOpen = false;
    ...
    public static void updateDoctor(DoctorPagePacket value) {
        ...
        // setStatus() patches the status element in place and records markup() as rendered. At this point
        // markup() already contains the new doctor packet, while the actual root still contains the loading
        // subtree. Invalidate that optimistic key so the scheduled pass must replace the stale root.
        current.renderedMarkupKey = "";
        // Let the view transition finish before replacing the loading subtree. AUI 1.2 can otherwise
        // retain the placeholder's first layout until an external resize forces another commit.
        current.scheduleRefresh(1);
    }
    ...
    public void tick() {
        ...
        if (doctorLayoutCommitTicks > 0 && --doctorLayoutCommitTicks == 0) {
            commitDoctorLayout();
        }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (!contextOpen && button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
            ...
            Element previewCard = previewCardActionAt(mouseX, mouseY);
            if (previewCard != null) {
                Document document = getLinkedDocument();
                if (document != null) document.setPressedElement(null);
                FindMeAuiSound.click();
                startClickPulse(previewCard, false);
                handleAction(previewCard.getAttribute("data-action"), previewCard.getAttribute("data-value"), previewCard);
                return true;
            }
        }
    ...
    private Element previewCardActionAt(double mouseX, double mouseY) {
        Document document = getLinkedDocument();
        if (document == null || view == View.DETAIL) return null;
        for (Element preview : document.querySelectorAll("findme-preview")) {
            Position position = Position.of(preview);
            Size size = Size.of(preview);
            if (size.width() <= 0.0 || size.height() <= 0.0
                    || mouseX < position.x || mouseX >= position.x + size.width()
                    || mouseY < position.y || mouseY >= position.y + size.height()) {
                continue;
            }
            Element card = preview.closest(".member-card[data-action]");
            if (card == null) card = preview.closest(".warehouse-card[data-action]");
    ...
    public void removed() {
        ...
        if (externalEditor != null && Minecraft.getInstance().screen != this) {
            externalEditor.onClose();
            externalEditor = null;
        }
        super.removed();
    ...
    @Override
    public void onClose() {
        if (warehouseSelectionMode && externalEditor == null) {
    ...
    private void settleSettingsDomMutation() {
        ...
        settleMainDomMutation();
        updateSettingsScrollThumb();
    }

    private void settleMainDomMutation() {
    ...
    private void updateStatusDom() {
        Document document = getLinkedDocument();
        if (document == null) return;
        Element element = document.getElementById("findme-status");
        if (element == null) return;
        element.setTextContent(status);
        element.setClassName("status" + (status.isBlank() ? " hidden" : ""));
        settleMainDomMutation();
    }
    ...
    private void commitDoctorLayout() {
        if (view != View.DOCTOR || doctor == null) return;
        Document document = getLinkedDocument();
    ...
    private void bindDocument() {
        ...
        if (!"1".equals(root.getAttribute("data-findme-bound"))) {
            ...
            root.addEventListener("click", event -> {
                ...
                com.kuzhi.findme.FindMeMod.LOGGER.debug("[FindMe backup-ui] main click action={} value={} view={} contextOpen={} transition={}",
            ...
            root.addEventListener("contextmenu", event -> {
                ...
                contextExternalSlotIndex = parseInt(card.getAttribute("data-member-index"), -1);
                contextTeam = -1;
                contextBackupIndex = -1;
                contextBackupSavedAt = 0L;
                settingsChoiceMenu = SettingsChoiceMenu.NONE;
            ...
            root.addEventListener("wheel", event -> {
                if (!(event instanceof MouseEvent mouse)) return;
                Element targetElement = eventTargetElement(event.target);
    ...
    private void bindContextOverlay() {
        ...
        root.addEventListener("click", event -> {
            ...
            if (action != null) {
                ...
                com.kuzhi.findme.FindMeMod.LOGGER.debug("[FindMe backup-ui] overlay click action={} value={} view={} contextOpen={} transition={}",
                        action.getAttribute("data-action"), action.getAttribute("data-value"), view, contextOpen, isTransitionRunning());
                handleAction(action.getAttribute("data-action"), action.getAttribute("data-value"), action);
                event.preventDefault();
                return;
        ...
        root.addEventListener("contextmenu", event -> {
            Element element = eventTargetElement(event.target);
            if (element == null || element.closest("[data-context-dismiss]") == null) return;
            contextOpen = false;
            settingsChoiceMenu = SettingsChoiceMenu.NONE;
            warehouseMenuPage = WarehouseMenuPage.MAIN;
            teamMenuPage = TeamMenuPage.MAIN;
    ...
    private void prepareDragGhost() {
        ...
        if (dragKind == DragKind.TEAM) {
            ...
            String compactState = externalEditor != null && view == View.TEAM && category == Category.COMPANION
                    ? externalEditor.primaryActionLabel(card.uuid, dragFrom) : stateLabel(card);
            ...
            dragGhostElement.setInnerHTML("<span class='member-number'>" + twoDigits(dragFrom + 1)
                    ...
                    + "</span><span class='compact-state'>" + escape(compactState) + "</span></div>");
        }
    ...
    private void startCardToggle(UUID uuid, int slotIndex, boolean expanding) {
        ...
        if (!settings.uiAnimations()) {
            ...
            if (isExternalTeamEditor()) expandedSlotIndex(expanding ? slotIndex : -1);
            refreshRosterCards();
            return;
        }
        Element card = findRenderedCard(uuid, slotIndex);
        if (card == null) {
            selectedUuid = uuid;
            expandedUuid = expanding ? uuid : null;
            if (isExternalTeamEditor()) expandedSlotIndex(expanding ? slotIndex : -1);
        ...
        clearOverlayMarkup();
        cardToggleUuid = uuid;
        cardToggleSlotIndex = isExternalTeamEditor() ? slotIndex : -1;
    ...
    private void refreshRosterCards() {
        ...
        for (Element element : roster.querySelectorAll(".member-card[data-uuid]")) {
            ...
            boolean selected = isExpandedCard(uuid, index);
            String state = card.active ? "active" : card.alive ? "stored" : "dead";
            ...
            if (settings.uiAnimations() && cardTogglePhase == CardTogglePhase.IN && cardToggleUuid != null) {
                toggleMotion = isToggleCard(uuid, index)
        ...
        // ApricityUI 1.2 commits pending style work at the start of the draw
        // pass. Flushing here as well can expose its white backing surface.
        document.reapplyStylesFromCache();
        renderedMarkupKey = markupKey(markup());
    ...
    private Element findRenderedCard(UUID uuid, int slotIndex) {
        if (!isExternalTeamEditor()) return findRenderedCard(uuid);
        Document document = getLinkedDocument();
        if (document == null || uuid == null) return null;
        for (Element element : document.querySelectorAll(".member-card[data-uuid]")) {
            if (!uuid.toString().equals(element.getAttribute("data-uuid"))) continue;
            if (parseInt(element.getAttribute("data-member-index"), -1) == slotIndex) return element;
        }
        return findRenderedCard(uuid);
    }
    ...
    private void handleAction(String action, String value, Element sourceElement) {
        ...
        com.kuzhi.findme.FindMeMod.LOGGER.debug("[FindMe backup-ui] handle action={} value={} view={} contextOpen={}",
        ...
        if (action.equals("back")) {
            if (view == View.DETAIL || view == View.SETTINGS || view == View.DOCTOR
                    || view == View.DEAD || view == View.RECOVERY) {
        ...
        if (action.equals("external-primary")) {
            UUID uuid = parseUuid(value);
            if (uuid != null && externalEditor != null) externalEditor.onPrimaryAction(uuid);
            return;
        }
        if (action.equals("external-action")) {
            if (selectedUuid != null && externalEditor != null) {
                externalEditor.onAction(selectedUuid, contextExternalSlotIndex, value == null ? "" : value);
                contextOpen = false;
                contextMotionClass = "";
                contextExternalSlotIndex = -1;
                refreshDocument();
        ...
        if (action.equals("settings-export-defaults")) {
            if (!ClientFindMeModuleState.canManageDefaults()) return;
            settingsDirty = false;
            ModNetwork.sendToServer(new com.kuzhi.findme.network.FindMeSettingsPacket(
        ...
        if (action.startsWith("doctor-filter:")) {
            try {
                BackupFilter targetFilter = BackupFilter.valueOf(
                        action.substring("doctor-filter:".length()).toUpperCase(Locale.ROOT));
                if (targetFilter == backupFilter && doctorView == DoctorView.BACKUPS) return;
                backupFilter = targetFilter;
                doctorView = DoctorView.BACKUPS;
                doctorRestoreConfirm = false;
                doctorBackupScrollTop = 0.0;
                refreshDocument();
        ...
        if (action.startsWith("backup-menu:")) {
            try {
                BackupMenuPage targetPage = BackupMenuPage.valueOf(
                        action.substring("backup-menu:".length()).toUpperCase(Locale.ROOT));
                backupMenuPage = targetPage;
                contextMotionClass = targetPage == BackupMenuPage.MAIN
                        ? "fm-context-back" : targetPage == BackupMenuPage.RENAME
        ...
        if (action.equals("select-warehouse-card")) {
            UUID uuid = parseUuid(value);
            if (uuid == null) return;
            if (externalEditor != null && warehouseSelectionMode) {
                externalEditor.onWarehouseSelection(uuid);
                warehouseSelectionMode = false;
                clearWarehouseSelectionDraft();
                view = View.TEAM;
                selectedUuid = uuid;
            ...
            if (warehouseSelectionMode) {
                toggleWarehouseSelection(uuid);
                return;
            }
            selectedUuid = uuid;
            expandedUuid = null;
            contextOpen = false;
            settleScrollableCardSelectionDomMutation();
            return;
        }
        ...
        if (action.equals("select-card")) {
            ...
            if (uuid == null) return;
            if (view == View.TEAM) {
                int slotIndex = parseInt(sourceElement == null ? "" : sourceElement.getAttribute("data-member-index"), -1);
                boolean sameExpanded = isExpandedCard(uuid, slotIndex);
                startCardToggle(uuid, slotIndex, !sameExpanded);
            } else {
                selectedUuid = uuid;
        ...
        switch (action) {
            ...
            case "replace-team-member" -> replaceTeamMember(uuid);
    ...
    private void settleScrollableCardSelectionDomMutation() {
        Document document = getLinkedDocument();
        if (document == null) return;
        for (Element card : document.querySelectorAll(".warehouse-card[data-uuid]")) {
            UUID uuid = parseUuid(card.getAttribute("data-uuid"));
            if (uuid != null && uuid.equals(selectedUuid)) addClass(card, "selected");
            else removeClass(card, "selected");
        }
        for (Element card : document.querySelectorAll(".death-card[data-uuid]")) {
            UUID uuid = parseUuid(card.getAttribute("data-uuid"));
            if (uuid != null && uuid.equals(selectedUuid)) addClass(card, "selected");
            else removeClass(card, "selected");
        }
    ...
    private void refreshSelectedTeamDom() {
        Document document = getLinkedDocument();
        if (document == null || view != View.TEAM) {
            refreshDocument();
            return;
        }
        Element workspace = document.querySelector(".fm-workspace");
        if (workspace == null) {
            refreshDocument();
            return;
        }
        for (Element row : document.querySelectorAll(".side-row[data-team-index]")) {
            int teamIndex = parseInt(row.getAttribute("data-team-index"), -1);
            if (teamIndex == selectedTeam) addClass(row, "active");
            else removeClass(row, "active");
        }
        List<ClientCompanionTeamState.TeamEntry> teams = teams();
        List<Card> visibleCards = cards();
        workspace.setInnerHTML(workspaceHeader(teams, visibleCards.size()) + rosterMarkup(visibleCards));
        document.rebuildSelectorIndex();
        document.reapplyStylesFromCache();
        settleMainDomMutation();
        updateTeamScrollThumb();
    ...
    private void beginWarehouseSelection() {
        if (externalEditor != null) {
            warehouseSelection.clear();
            return;
        }
        warehouseSelection.begin(ClientCompanionTeamState.members(target(), selectedTeam),
                allCards().stream().filter(this::acceptedWarehouseCard).map(Card::uuid).toList());
    }

    private void replaceTeamMember(UUID uuid) {
        if (uuid == null || externalEditor != null) return;
        view = View.WAREHOUSE;
        warehouseFilter = WarehouseFilter.ALL;
        warehouseSelectionMode = true;
        beginWarehouseSelection();
        warehouseSelection.toggle(uuid);
        warehouseScrollTop = 0.0;
        search = "";
        selectedUuid = null;
        expandedUuid = null;
    ...
    private void clearWarehouseSelectionDraft() {
        warehouseSelection.clear();
    }

    private void cancelWarehouseSelection() {
        view = View.TEAM;
        warehouseSelectionMode = false;
        clearWarehouseSelectionDraft();
        selectedUuid = null;
    ...
    private void toggleWarehouseSelection(UUID uuid) {
        if (uuid == null || externalEditor != null) return;
        if (!warehouseSelection.toggle(uuid)) {
            setStatus(tr("screen.find_me.aui.team_selection_full"), 50);
            return;
        }
        selectedUuid = null;
        settleWarehouseSelectionDomMutation();
    }

    private void settleWarehouseSelectionDomMutation() {
        Document document = getLinkedDocument();
        if (document == null) return;
        boolean full = warehouseSelection.full();
        for (Element card : document.querySelectorAll(".warehouse-card[data-uuid]")) {
            UUID uuid = parseUuid(card.getAttribute("data-uuid"));
            boolean selected = uuid != null && warehouseSelection.contains(uuid);
            if (selected) {
                addClass(card, "selected");
                addClass(card, "draft-selected");
            } else {
                removeClass(card, "selected");
                removeClass(card, "draft-selected");
            }
            if (full && !selected) addClass(card, "selection-full");
            else removeClass(card, "selection-full");
        }

        Element sidebarCount = document.getElementById("findme-warehouse-sidebar-selection-count");
        if (sidebarCount != null) sidebarCount.setTextContent(twoDigits(warehouseSelection.size()));
        Element footerCount = document.getElementById("findme-warehouse-selection-count");
        if (footerCount != null) {
            footerCount.setTextContent(tr("screen.find_me.aui.team_selection_count",
                    warehouseSelection.size(), FindMeUiSettings.TEAM_CAPACITY));
        }
        Element confirm = document.getElementById("findme-warehouse-confirm");
        boolean canConfirm = externalEditor == null && warehouseSelectionChanged();
        if (confirm != null) {
            confirm.setClassName("ui-button warehouse-confirm " + (canConfirm ? "primary" : "disabled"));
        }
    ...
    private void confirmWarehouseSelection() {
        if (!warehouseSelectionMode || externalEditor != null || !warehouseSelectionChanged()) return;
        CompanionTeamTarget currentTarget = target();
        List<UUID> members = warehouseSelection.selected();
        UUID inserted = members.stream().filter(uuid -> !warehouseSelection.original().contains(uuid))
                .findFirst().orElse(null);
        for (UUID uuid : members) removeFromClientTeams(currentTarget, uuid);
        ClientCompanionTeamState.replaceMembers(currentTarget, selectedTeam, members);
        ModNetwork.sendToServer(new CompanionTeamCommandPacket(CompanionTeamAction.SET,
                currentTarget, selectedTeam, -1, "", members));
        pendingInsertedMemberUuid = inserted;
        view = View.TEAM;
        warehouseSelectionMode = false;
        clearWarehouseSelectionDraft();
        selectedUuid = inserted;
        expandedUuid = null;
        search = "";
        warehouseScrollTop = 0.0;
        contextOpen = false;
    ...
    private boolean warehouseSelectionChanged() {
        return warehouseSelection.changed();
    }
    ...
    private void refreshDocument() {
        ...
        if (oldWarehouseGrid != null) warehouseScrollTop = oldWarehouseGrid.getTargetScrollTop();
        ...
        if (view == View.DOCTOR) {
            com.kuzhi.findme.FindMeMod.LOGGER.debug("[FindMe backup-ui] doctor refresh begin doctor={} doctorView={} markupChanged={} layoutChanged={} markupLength={} transition={}",
        ...
        if (markupChanged) {
            ...
            if (view == View.DOCTOR && doctor != null && doctorView == DoctorView.BACKUPS) {
                ...
                com.kuzhi.findme.FindMeMod.LOGGER.debug(
            ...
            // AUI 1.2 commits pending style work immediately before drawing.
            // Avoid a synchronous full-document flush here: during a category
            // transfer it can expose AUI's white backing surface for a frame.
            document.reapplyStylesFromCache();
            ...
            if (view == View.DOCTOR && doctor != null) doctorLayoutCommitTicks = 1;
        }
        ...
        if (view == View.DOCTOR) {
            ...
            com.kuzhi.findme.FindMeMod.LOGGER.debug("[FindMe backup-ui] doctor refresh end page={} pageBox={}x{} sidebar={} content={}x{} records={}",
        ...
        schedulePendingScrollRestore(teamList, warehouseGrid, settingsGrid, backupRecords);
        postworkPhaseStartedAt = FindMeAuiPerformanceMonitor.start();
        refreshContextOverlay();
    ...
    private void schedulePendingScrollRestore(Element teamList, Element warehouseGrid,
                                              Element settingsGrid, Element backupRecords) {
        pendingTeamScrollRestore = teamList == null ? Double.NaN : teamScrollTop;
        pendingWarehouseScrollRestore = warehouseGrid == null ? Double.NaN : warehouseScrollTop;
        pendingSettingsScrollRestore = settingsGrid == null ? Double.NaN : settingsScrollTop;
        pendingDoctorBackupScrollRestore = backupRecords == null ? Double.NaN : doctorBackupScrollTop;
        scrollRestoreTicks = Double.isNaN(pendingTeamScrollRestore)
                && Double.isNaN(pendingWarehouseScrollRestore)
                && Double.isNaN(pendingSettingsScrollRestore)
                && Double.isNaN(pendingDoctorBackupScrollRestore) ? 0 : 3;
    }

    private void applyPendingScrollRestore() {
        if (scrollRestoreTicks <= 0) return;
        Document document = getLinkedDocument();
        if (document == null) {
            scrollRestoreTicks = 0;
            return;
        }
        restoreScrollTop(document.getElementById("findme-team-list"), pendingTeamScrollRestore);
        restoreScrollTop(document.getElementById("findme-warehouse-grid"), pendingWarehouseScrollRestore);
        restoreScrollTop(document.getElementById("findme-settings-grid"), pendingSettingsScrollRestore);
        restoreScrollTop(document.getElementById("findme-doctor-backup-records"), pendingDoctorBackupScrollRestore);
        scrollRestoreTicks--;
    }

    private static void restoreScrollTop(Element element, double scrollTop) {
        if (element != null && !Double.isNaN(scrollTop)
                && Math.abs(element.getTargetScrollTop() - scrollTop) > 0.01) {
    ...
    private void updateWarehouseScrollThumb() {
        if (view != View.WAREHOUSE && view != View.DEAD && view != View.RECOVERY) return;
        Document document = getLinkedDocument();
        if (document == null) return;
    ...
    private void updateDoctorBackupScrollThumb() {
        ...
        double contentHeight = Math.max(viewportHeight, Size.getContentSize(records).height());
        double scrollLimit = Math.max(0.0, contentHeight - viewportHeight);
        double target = records.getTargetScrollTop();
        double current = Math.max(0.0, Math.min(scrollLimit, target));
        if (current != target) records.setScrollTop(current);
    ...
    private String markup() {
        ...
        String title = externalEditor != null && view == View.TEAM && category == Category.COMPANION
                ? externalEditor.title()
                : view == View.DEAD ? tr("screen.find_me.aui.deaths")
        ...
        String subtitle = externalEditor != null && view == View.TEAM && category == Category.COMPANION
                ? tr("screen.find_me.aui.team_workspace.secondary")
                : view == View.DEAD ? "MEMORIAL ARCHIVE"
                : view == View.RECOVERY ? "DATA RECOVERY"
                : view == View.WAREHOUSE ? "COMPANION STORAGE" : tr("screen.find_me.aui.manage_title.secondary");
        StringBuilder html = new StringBuilder("<div class='fm-page ").append(pageClass)
                .append(warehouseSelectionMode ? " warehouse-selection-mode" : "").append(" ").append(typographyClasses())
                .append("' style='height:").append(Math.max(1, height)).append("px'>")
                .append(headerMarkup(title, subtitle, externalEditor == null && !warehouseSelectionMode));
        ...
        html.append("<div id='findme-status' class='status").append(status.isBlank() ? " hidden" : "")
                .append("'>").append(escape(status)).append("</div>");
    ...
    private String sidebarMarkup(List<ClientCompanionTeamState.TeamEntry> teams) {
        ...
        if (view == View.TEAM) {
            ...
            if (warehouseSelectionMode && externalEditor == null) {
                html.append(sideCount("fm-side-stage-1 active", warehouseSelection.size(),
                        tr("screen.find_me.aui.team_selection"), "SELECTED / 06", "",
                        "findme-warehouse-sidebar-selection-count"));
            } else if (backupWarehousePreview) {
                html.append(sideCount("fm-side-stage-1 active", all, tr("screen.find_me.aui.backups"),
                        "BACKUP SNAPSHOT", "doctor-view-backups"));
            } else {
    ...
    private String sideCount(String classes, int count, String label, String subtitle, String action) {
        return sideCount(classes, count, label, subtitle, action, "");
    }

    private String sideCount(String classes, int count, String label, String subtitle, String action,
                             String numberId) {
        String actionAttribute = action == null || action.isBlank()
                ? "" : " data-action='" + escape(action) + "'";
        String idAttribute = numberId == null || numberId.isBlank()
                ? "" : " id='" + escape(numberId) + "'";
        return "<div class='side-row " + classes + "'" + actionAttribute + "><b" + idAttribute + " class='"
                + (count >= 100 ? "wide-count" : "") + "'>" + twoDigits(count)
                + "</b><span><strong>" + escape(label) + "</strong><small>" + subtitle
                + "</small></span></div>";
    }
    ...
    private String workspaceHeader(List<ClientCompanionTeamState.TeamEntry> teams, int count) {
        ...
        String subtitle = view == View.TEAM ? tr("screen.find_me.aui.team_workspace.secondary")
                : view == View.WAREHOUSE ? (backupWarehousePreview
                        ? "BACKUP SNAPSHOT" : warehouseFilter.subtitle)
    ...
    private String warehouseMarkup(List<Card> visibleCards) {
        ...
        for (int i = 0; i < visibleCards.size(); i++) {
            ...
            boolean draftSelected = warehouseSelectionMode && warehouseSelection.contains(card.uuid());
            boolean selectionFull = warehouseSelectionMode && warehouseSelection.full() && !draftSelected;
            html.append("<div class='warehouse-card").append(draftSelected || card.uuid().equals(selectedUuid) ? " selected" : "")
                    .append(draftSelected ? " draft-selected" : "")
                    .append(selectionFull ? " selection-full" : "")
            ...
            html.append(" data-preview-scale='0.68' data-preview-overlay='warehouse' data-preview-viewport='findme-warehouse-grid'")
                    .append(" data-preview-name='").append(escape(card.name)).append("' data-preview-team='").append(escape(team))
                    .append("' data-preview-number='").append(snapshot ? twoDigits(i + 1) : "")
                    .append("' data-preview-status='").append(snapshot && !card.alive
                            ? escape(tr("screen.find_me.aui.death_record")) : card.active()
                            ? escape(tr("screen.find_me.manage.state_deployed")) : "")
                    .append("' data-preview-status-tone='").append(snapshot && !card.alive ? "death" : "")
        ...
        if (warehouseSelectionMode) {
            html.append("<div id='findme-warehouse-selection-count' class='warehouse-selection-count'>")
                    .append(escape(tr("screen.find_me.aui.team_selection_count", warehouseSelection.size(),
                            FindMeUiSettings.TEAM_CAPACITY))).append("</div>");
            boolean canConfirm = externalEditor == null && warehouseSelectionChanged();
            html.append("<div id='findme-warehouse-confirm' class='ui-button warehouse-confirm ")
                    .append(canConfirm ? "primary" : "disabled")
                    .append("' data-action='confirm-warehouse-selection'>")
                    .append(escape(tr("screen.find_me.aui.confirm_add"))).append("</div>");
        }
        return html.append("</div>").toString();
    }
    ...
    private String rosterCardsMarkup(List<Card> visibleCards) {
        ...
        for (int i = 0; i < visibleCards.size(); i++) {
            Card card = visibleCards.get(i);
            int memberIndex = externalEditor != null && view == View.TEAM
                    ? i : view == View.TEAM ? teamMembers.indexOf(card.uuid) : i;
    ...
    private String cardMarkup(Card card, int index, String compactStyle, String selectedStyle) {
        String state = card.active ? "active" : card.alive ? "stored" : "dead";
        boolean selected = isExpandedCard(card.uuid, index);
        ...
        String cardState = externalEditor != null && category == Category.COMPANION && view == View.TEAM
                ? externalEditor.primaryActionLabel(card.uuid, index) : stateLabel(card);
        String primary = externalEditor != null && category == Category.COMPANION && view == View.TEAM
                ? button("external-primary", externalEditor.primaryActionLabel(card.uuid, index), "primary selected-action")
                        .replace("data-action='external-primary'", "data-action='external-primary' data-value='" + card.uuid + "'")
                : view == View.WAREHOUSE
        ...
        String compactPane = "<div class='compact-pane'><span class='member-number'>" + twoDigits(index + 1)
                + "</span><div class='compact-info'><span class='compact-name'>" + escape(card.name)
                + "</span><span class='compact-state'>" + escape(cardState) + "</span></div></div>";
        return "<div class='member-card " + (selected ? "selected-card " : "compact ") + state
                ...
                + compactPane
                + "<findme-preview data-uuid='" + card.uuid + "' data-preview-overlay='expanded'"
                + " data-preview-overlay-height='" + scaled(141) + "'"
                + " data-preview-name='" + escape(card.name) + "' data-preview-type='" + escape(card.type)
                + "' data-preview-team='" + escape(cardState) + " &#183; " + escape(tr(category.key))
                + "' data-preview-number='" + twoDigits(index + 1) + "' data-preview-status='" + escape(deployed)
                + "'></findme-preview>"
                + spellSlot
    ...
    private String detailPageMarkup() {
        ...
        html.append("<div class='profile-model'><div class='profile-preview-background'></div><findme-preview data-uuid='").append(card.uuid)
                .append("' data-interaction-id='manage-detail' data-preview-overlay='profile' data-preview-name='")
                .append(escape(card.name)).append("' data-preview-type='").append(escape(card.type))
                .append("' data-preview-team='").append(escape(tr(category.key))).append(" / ")
                .append(escape(movement)).append("'></findme-preview></div>");
    ...
    private String settingsMarkup() {
        ...
        String[] sectionEnglishLabels = {"INTERFACE", "TEAMS", "RIDING"};
        ...
        for (int i = 0; i < sectionLabelKeys.length; i++) {
            html.append("<div class='system-row system-side-stage-").append(Math.min(3, i + 1)).append(" ")
                    ...
                    .append(sectionEnglishLabels[i]).append("</small></span></div>");
        }
        html.append("</div></div><div class='settings-content'><div class='system-head'><b>").append(twoDigits(settingsSection + 1))
                ...
                .append(sectionEnglishLabels[settingsSection]).append(" / FINDME SETTINGS")
                .append("</small><strong>")
                .append(escape(tr(sectionKeys[settingsSection]))).append("</strong></span>");
        if (ClientFindMeModuleState.canManageDefaults()) {
            html.append(button("settings-export-defaults", tr("screen.find_me.aui.setting.set_defaults"), "settings-export"))
                    .append(button("settings-reset", tr("screen.find_me.aui.setting.reset_all"), "settings-reset"));
        } else {
            html.append(button("settings-reset", tr("screen.find_me.config_reset"), "settings-reset"));
        }
    ...
    private String doctorMarkup() {
        ...
        if (doctor == null) return html.append("<div class='doctor-loading'>")
                .append(button("doctor-refresh", tr("screen.find_me.aui.refresh"), "doctor-refresh-button"))
                .append("</div>").toString();
    ...
    private String doctorSidebarMarkup() {
        StringBuilder html = new StringBuilder("<div class='fm-sidebar'><div class='fm-sidebar-sheet'></div><div class='fm-sidebar-content'>")
                .append("<div class='side-heading fm-side-stage-0'><strong>")
                .append(escape(tr("screen.find_me.doctor.backup_types"))).append("</strong><small>")
                .append(escape(tr("screen.find_me.doctor.caption.backup_types"))).append("</small></div>");
        int index = 0;
        for (BackupFilter filter : BackupFilter.values()) {
            html.append("<div class='side-row fm-side-stage-").append(Math.min(3, index + 1))
                    .append(filter == backupFilter ? " active" : "")
                    .append("' data-action='doctor-filter:").append(filter.name().toLowerCase(Locale.ROOT))
                    .append("'><b>").append(twoDigits(index + 1)).append("</b><span><strong>")
                    .append(escape(tr(filter.titleKey))).append("</strong><small>")
                    .append(escape(tr(filter.captionKey))).append("</small></span></div>");
            index++;
        }
        return html.append("<div class='side-spacer'></div></div></div>").toString();
    }

    private String doctorBackupsMarkup() {
        List<DoctorPagePacket.Backup> backups = filteredDoctorBackups();
        String action = backupFilter == BackupFilter.MANUAL
                ? button("doctor-backup", tr("screen.find_me.aui.create_backup"), "doctor-create-button") : "";
        StringBuilder html = new StringBuilder(doctorHeader(twoDigits(backupFilter.ordinal() + 1), backupFilter.titleKey,
                escape(tr(backupFilter.captionKey)), action))
        ...
        if (backups.isEmpty()) html.append("<div class='doctor-empty-report'><b>00</b><span>")
                .append(escape(tr(backupFilter.emptyKey))).append("</span></div>");
        return html.append("</div>").toString();
    }

    private List<DoctorPagePacket.Backup> filteredDoctorBackups() {
        if (doctor == null) return List.of();
        return doctor.backups().stream().filter(backupFilter::matches).toList();
    }
    ...
    private String doctorBackupDetailMarkup() {
        DoctorPagePacket.Backup backup = doctor.backups().stream()
                ...
                .orElse(null);
        if (backup == null) {
            return doctorHeader(twoDigits(backupFilter.ordinal() + 1), backupFilter.titleKey,
                    escape(tr(backupFilter.captionKey)), "")
                    + "<div class='doctor-detail-placeholder'><b>00</b><span>"
                    + escape(tr("message.find_me.backup_empty")) + "</span></div>";
        }
        StringBuilder html = new StringBuilder(doctorHeader(twoDigits(backupFilter.ordinal() + 1), backupFilter.titleKey,
                escape(tr("screen.find_me.doctor.caption.snapshot")),
    ...
    private String doctorHeader(String number, String titleKey, String caption, String action) {
        return "<div class='doctor-head'><b>" + number + "</b><span><strong>" + escape(tr(titleKey))
                + "</strong><small>" + caption + "</small></span>" + action + "</div>";
    }
    ...
    private String backupDisplayName(DoctorPagePacket.Backup backup) {
        if (BackupFilter.AUTOMATIC.matches(backup)) return tr("screen.find_me.doctor.automatic_backup");
        String reason = backup.reason();
        if (backup.manual() && (reason == null || reason.isBlank() || "manual".equalsIgnoreCase(reason)
                || "aui_manual".equalsIgnoreCase(reason))) {
            return tr("screen.find_me.doctor.manual_backup");
        }
        String displayReason = backupReason(reason);
        return displayReason.isBlank() ? tr("screen.find_me.doctor.other_backup") : displayReason;
    }

    private String backupRenameValue(DoctorPagePacket.Backup backup) {
        String reason = backup.reason();
        return reason == null || reason.isBlank() || "manual".equalsIgnoreCase(reason)
                || "aui_manual".equalsIgnoreCase(reason)
                ? tr("screen.find_me.doctor.manual_backup") : reason;
    }
    ...
    private String contextMarkup() {
        ...
        if (spellPickerUuid != null) {
            ...
            if (pendingDanger.equals("backup")) {
            ...
            } else if (backupMenuPage == BackupMenuPage.ACTIONS) {
                ...
                html.append(button("backup-menu:main", tr("screen.find_me.aui.back"), "ui-button menu-button"));
            } else if (backupMenuPage == BackupMenuPage.RENAME && backup != null && backup.manual()) {
                html.append("<span class='menu-title'>").append(escape(tr("screen.find_me.aui.rename"))).append("</span>")
                        ...
                        .append(escape(backupRenameValue(backup))).append("' maxlength='48'>")
                        .append(button("doctor-backup-rename", tr("screen.find_me.config_save"), "ui-button menu-button"))
                        .append(button("backup-menu:main", tr("screen.find_me.aui.back"), "ui-button menu-button"));
            } else {
                html.append("<span class='menu-title'>")
                        .append(escape(tr("screen.find_me.aui.more_actions"))).append("</span>")
                        .append(button("backup-menu:actions", tr("screen.find_me.aui.backup_actions"), "ui-button menu-button"));
                if (backup != null && backup.manual()) {
                    html.append(button("backup-menu:rename", tr("screen.find_me.aui.rename"), "ui-button menu-button"));
                }
            }
            ...
            if (view == View.WAREHOUSE && selected != null) {
            ...
            } else if (externalEditor != null && view == View.TEAM && category == Category.COMPANION && selected != null) {
                html.append("<span class='menu-title'>").append(escape(externalEditor.title())).append("</span>");
                List<FindMeExternalCompanionEditor.Action> actions = externalEditor.actions(selectedUuid, contextExternalSlotIndex);
                String currentActionId = externalEditor.currentActionId(selectedUuid, contextExternalSlotIndex);
                boolean hasAction = false;
                if (actions != null) {
                    for (FindMeExternalCompanionEditor.Action action : actions) {
                        if (action == null || action.id() == null || action.id().isBlank()) continue;
                        hasAction = true;
                        String actionClasses = "ui-button menu-button"
                                + (action.id().equals(currentActionId) ? " active" : "");
                        html.append(button("external-action", action.label() == null ? action.id() : action.label(),
                                actionClasses).replace("data-action='external-action'",
                                "data-action='external-action' data-value='" + escape(action.id()) + "'"));
                    }
                }
                if (!hasAction) {
                    html.append("<div class='spell-picker-empty'>")
                            .append(escape(tr("screen.find_me.aui.no_records"))).append("</div>");
                }
                ...
                if (view == View.TEAM) {
                    html.append(button("replace-team-member", tr("screen.find_me.aui.replace_team_member"), "ui-button menu-button")
                            .replace("data-action='replace-team-member'", "data-action='replace-team-member' data-value='" + selectedUuid + "'"));
                }
    ...
    private List<Card> cards() {
        ...
        if (view == View.WAREHOUSE) {
            if (warehouseSelectionMode && externalEditor == null) {
                List<Card> all = allCards().stream().filter(this::acceptedWarehouseCard)
                        .filter(this::matches).toList();
                List<Card> sorted = new ArrayList<>();
                for (UUID uuid : warehouseSelection.displayOrder()) {
                    all.stream().filter(card -> card.uuid().equals(uuid)).findFirst().ifPresent(sorted::add);
                }
                all.stream().filter(card -> sorted.stream().noneMatch(existing -> existing.uuid().equals(card.uuid())))
                        .forEach(sorted::add);
                return sorted;
            }
            return switch (warehouseFilter) {
                case ALL -> allCards().stream().filter(this::acceptedWarehouseCard).filter(this::matches).toList();
                case ASSIGNED -> assignedCards();
                case UNASSIGNED -> unassignedCards();
            };
        }
        List<Card> result = new ArrayList<>();
        if (externalEditor != null && category == Category.COMPANION && view == View.TEAM) {
            for (UUID uuid : externalEditor.editorSlots(selectedTeam)) {
                Card card = findCard(uuid);
        ...
        for (UUID uuid : members) {
            Card card = findCard(uuid);
            if (card != null && (externalEditor == null || category != Category.COMPANION
                    || externalEditor.acceptsWarehouseType(card.type())) && matches(card)) result.add(card);
        }
        return result;
    }
    ...
    private List<Card> unassignedCards() {
        Set<UUID> assigned = assignedUuids();
        return allCards().stream().filter(this::acceptedWarehouseCard)
                .filter(card -> !assigned.contains(card.uuid())).filter(this::matches).toList();
    }

    private boolean isExternalTeamEditor() {
        return externalEditor != null && category == Category.COMPANION && view == View.TEAM;
    }

    private int expandedSlotIndex = -1;

    private void expandedSlotIndex(int value) {
        expandedSlotIndex = value;
    }

    private boolean isExpandedCard(UUID uuid, int slotIndex) {
        return isExternalTeamEditor()
                ? slotIndex >= 0 && slotIndex == expandedSlotIndex
                : uuid != null && uuid.equals(expandedUuid);
    }

    private boolean isToggleCard(UUID uuid, int slotIndex) {
        return isExternalTeamEditor() ? slotIndex >= 0 && slotIndex == cardToggleSlotIndex
                : uuid != null && uuid.equals(cardToggleUuid);
    }
    ...
    private boolean acceptedWarehouseCard(Card card) {
        return externalEditor == null || category != Category.COMPANION
                || externalEditor.acceptsWarehouseType(card.type());
    }
    ...
    private List<Card> allCards() {
        ...
        if (category == Category.VEHICLE && externalEditor == null) ClientVehicleState.allEntries().forEach(entry -> result.add(new Card(entry.uuid(), entry.name(), entry.entityType(), entry.loaded() && entry.alive(), entry.alive(), CompanionMoveType.WALK, entry.asPreviewEntry())));
        else if (externalEditor != null) {
            ClientCompanionState.allEntries(CompanionKind.COMPANION)
            ...
            ClientCompanionState.allEntries(CompanionKind.MOUNT)
                    .forEach(entry -> result.add(new Card(entry.uuid(), entry.name(), entry.entityType(), entry.loaded() && entry.alive(), entry.alive(), entry.moveType(), entry)));
        } else {
            CompanionKind kind = target() == CompanionTeamTarget.MOUNT ? CompanionKind.MOUNT : CompanionKind.COMPANION;
            ClientCompanionState.allEntries(kind)
                    .forEach(entry -> result.add(new Card(entry.uuid(), entry.name(), entry.entityType(), entry.loaded() && entry.alive(), entry.alive(), entry.moveType(), entry)));
        }
        return result;
    }
    ...
    private Card findCard(UUID uuid) {
        ...
        if (category == Category.VEHICLE && externalEditor == null) {
            ...
            for (var entry : ClientCompanionState.allEntries(CompanionKind.COMPANION)) {
                if (entry.uuid().equals(uuid)) return new Card(uuid, entry.name(), entry.entityType(), entry.loaded() && entry.alive(), entry.alive(), entry.moveType(), entry);
            }
            for (var entry : ClientCompanionState.allEntries(CompanionKind.MOUNT)) {
                if (entry.uuid().equals(uuid)) return new Card(uuid, entry.name(), entry.entityType(), entry.loaded() && entry.alive(), entry.alive(), entry.moveType(), entry);
            }
        } else {
            CompanionKind kind = target() == CompanionTeamTarget.MOUNT ? CompanionKind.MOUNT : CompanionKind.COMPANION;
    ...
    private CompanionTeamTarget target() {
        if (externalEditor != null) return externalEditor.teamTarget();
    ...
    private enum BackupFilter {
        AUTOMATIC("screen.find_me.doctor.automatic_backup", "screen.find_me.doctor.caption.automatic",
                "screen.find_me.doctor.empty.automatic"),
        MANUAL("screen.find_me.doctor.manual_backup", "screen.find_me.doctor.caption.manual",
                "screen.find_me.doctor.empty.manual"),
        OTHER("screen.find_me.doctor.other_backup", "screen.find_me.doctor.caption.other",
                "screen.find_me.doctor.empty.other");

        final String titleKey;
        final String captionKey;
        final String emptyKey;

        BackupFilter(String titleKey, String captionKey, String emptyKey) {
            this.titleKey = titleKey;
            this.captionKey = captionKey;
            this.emptyKey = emptyKey;
        }

        boolean matches(DoctorPagePacket.Backup backup) {
            if (backup == null) return false;
            if (this == MANUAL) return backup.manual();
            boolean automatic = !backup.manual() && "auto".equalsIgnoreCase(backup.reason());
            return this == AUTOMATIC ? automatic : !backup.manual() && !automatic;
        }
    }
    ...
    private enum BackupMenuPage { MAIN, ACTIONS, RENAME }
