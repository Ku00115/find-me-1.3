...
public record StorageEffectPacket(int entityId, double x, double y, double z, float radius, float height, int durationTicks, Visual visual) implements CustomPacketPayload {
    public static final CustomPacketPayload.Type<StorageEffectPacket> TYPE = new CustomPacketPayload.Type<>(ResourceLocation.fromNamespaceAndPath(FindMeMod.MODID, "storage_effect"));
    public static final StreamCodec<RegistryFriendlyByteBuf, StorageEffectPacket> STREAM_CODEC = NetworkCodecs.of(StorageEffectPacket::encode, StorageEffectPacket::decode);
