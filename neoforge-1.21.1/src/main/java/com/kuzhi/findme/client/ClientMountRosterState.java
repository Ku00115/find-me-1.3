...
public final class ClientMountRosterState {
    ...
    public static List<Entry> entries() {
        boolean includeCobblemon = CobblemonCompat.available()
                && ClientFindMeModuleState.enabled(FindMeModule.COBBLEMON_INTEGRATION);
        List<Entry> cobblemon = new ArrayList<>();
        if (includeCobblemon) appendCobblemon(cobblemon);
        List<Entry> findMe = new ArrayList<>();
        appendFindMe(findMe);
        return composeSections(includeCobblemon, cobblemon, findMe, List.of());
    }

    static List<Entry> composeSections(boolean includeCobblemon, List<Entry> cobblemon,
                                       List<Entry> findMe, List<Entry> vehicles) {
        List<Entry> result = new ArrayList<>();
        if (includeCobblemon && cobblemon != null) result.addAll(cobblemon);
    ...
    private static void appendCobblemon(List<Entry> result) {
        Map<Integer, CobblemonPartyPacket.Entry> bySlot = new HashMap<>();
        for (CobblemonPartyPacket.Entry entry : ClientCobblemonState.entries()) bySlot.put(entry.slot(), entry);
        for (int slot = 0; slot < 6; slot++) {
            CobblemonPartyPacket.Entry pokemon = bySlot.get(slot);
            result.add(pokemon == null ? Entry.placeholder(MountRosterSource.COBBLEMON, slot)
                    : Entry.cobblemon(pokemon));
        }
    }
    ...
    public record Entry(MountRosterSource source, UUID uuid, int sourceSlot, int teamIndex,
                        ...
                        CobblemonPartyPacket.Entry pokemon) {
        static Entry findMe(CompanionListPacket.Entry entry, int sourceSlot, int teamIndex) {
            return new Entry(MountRosterSource.FIND_ME, entry.uuid(), sourceSlot, teamIndex, entry, null, null);
        }

        static Entry vehicle(VehicleListPacket.Entry entry, int sourceSlot, int teamIndex) {
            return new Entry(MountRosterSource.VEHICLE, entry.uuid(), sourceSlot, teamIndex, null, entry, null);
        }

        static Entry cobblemon(CobblemonPartyPacket.Entry entry) {
            return new Entry(MountRosterSource.COBBLEMON, entry.uuid(), entry.slot(), -1, null, null, entry);
        }

        static Entry placeholder(MountRosterSource source, int sourceSlot) {
            return new Entry(source, null, sourceSlot, -1, null, null, null);
        }

        public boolean empty() { return uuid == null; }
        public Key key() { return empty() ? null : new Key(source, uuid); }
        public boolean deployed() {
            return pokemon != null ? pokemon.deployed()
                    : vehicle != null ? vehicle.deployed() || vehicle.ridden()
                    : findMe != null && (findMe.deployed() || findMe.ridden());
        }
        public boolean ridden() {
            return pokemon != null ? pokemon.ridden()
                    : vehicle != null ? vehicle.ridden() : findMe != null && findMe.ridden();
        }
        public boolean alive() {
            return pokemon != null ? !pokemon.fainted()
                    : vehicle != null ? vehicle.alive() : findMe != null && findMe.alive();
        }
        public String name() {
            return pokemon != null ? pokemon.name() : vehicle != null ? vehicle.name()
                    : findMe == null ? "" : findMe.name();
        }
        public CompanionListPacket.Entry asPreviewEntry() {
            return pokemon != null ? pokemon.asPreviewEntry()
                    : vehicle != null ? vehicle.asPreviewEntry() : findMe;
        }
    }
