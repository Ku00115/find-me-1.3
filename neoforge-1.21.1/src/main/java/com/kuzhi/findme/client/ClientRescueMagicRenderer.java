...
final class ClientRescueMagicRenderer {
    ...
    static void renderWorld(RenderLevelStageEvent event) {
        ...
        for (ClientRescueMagicRenderState.Effect effect : ClientRescueMagicRenderState.effects()) {
            if (effect.customTexture) {
                renderCustomEffect(matrix, effect, event.getPartialTick().getGameTimeDeltaPartialTick(false));
            }
        }
        renderKineticEffects(matrix, event.getPartialTick().getGameTimeDeltaPartialTick(false));
    ...
    private static void renderKineticEffects(Matrix4f matrix, float partialTick) {
        ...
        RenderSystem.enableDepthTest();
        BufferBuilder ringBuffer = Tesselator.getInstance().begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
        for (ClientRescueMagicRenderState.Effect effect : ClientRescueMagicRenderState.effects()) {
    ...
    private static void vertex(BufferBuilder buffer, Matrix4f matrix, Vec3 position, Color color, float alpha) {
        buffer.addVertex(matrix, (float)position.x, (float)position.y, (float)position.z)
                .setColor(color.red(), color.green(), color.blue(), Mth.clamp(alpha, 0.0f, 1.0f));
    }
