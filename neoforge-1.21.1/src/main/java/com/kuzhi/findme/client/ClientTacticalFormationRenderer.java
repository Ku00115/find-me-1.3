...
final class ClientTacticalFormationRenderer {
    ...
    static void renderWorld(RenderLevelStageEvent event) {
        ...
        PoseStack pose = event.getPoseStack(); Camera camera = event.getCamera(); Vec3 cameraPos = camera.getPosition();
        pose.pushPose(); pose.translate(-cameraPos.x, -cameraPos.y, -cameraPos.z); Matrix4f matrix = pose.last().pose();
        RenderSystem.enableBlend(); RenderSystem.defaultBlendFunc(); RenderSystem.disableCull();
        RenderSystem.enableDepthTest(); RenderSystem.depthMask(false); RenderSystem.setShader(GameRenderer::getPositionColorShader);
        BufferBuilder buffer = Tesselator.getInstance().begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.POSITION_COLOR);
        for (ClientTacticalFormationState.Effect effect : ClientTacticalFormationState.effects())
            renderEffect(buffer, matrix, effect, event.getPartialTick().getGameTimeDeltaPartialTick(false));
        BufferUploader.drawWithShader(buffer.buildOrThrow());
        RenderSystem.depthMask(true); RenderSystem.enableCull(); RenderSystem.disableBlend(); pose.popPose();
    }
    private static void renderEffect(BufferBuilder buffer, Matrix4f matrix,
        ...
        float age = effect.age + partialTick, progress = Mth.clamp(age / effect.durationTicks, 0.0f, 1.0f);
        float fade = 1.0f - smooth(Mth.clamp((progress - 0.78f) / 0.22f, 0.0f, 1.0f));
        float centerOpen = smooth(Mth.clamp(age / 8.0f, 0.0f, 1.0f)), alpha = centerOpen * fade;
        ClientMagicCircleRenderer.renderGroundCircle(buffer, matrix, effect.center,
                effect.centerRadius * (0.15 + centerOpen * 0.85), BLUE, 0.72f * alpha, progress, age * 1.1f, 0.48);
        for (CompanionTacticalFormationPacket.Member member : effect.members) {
            float localAge = age - member.revealOffsetTicks(); if (localAge < 0.0f) continue;
            float open = smooth(Mth.clamp(localAge / 7.0f, 0.0f, 1.0f));
            Vec3 ground = new Vec3(member.x(), member.y() + 0.08, member.z());
            ClientMagicCircleRenderer.line(buffer, matrix, effect.center.add(0.0, 0.10, 0.0), ground,
                    Math.max(0.025, member.radius() * 0.014), WHITE, 0.60f * open * fade);
            Vec3 circle = new Vec3(member.x(), member.y() + Math.max(3.0, member.radius() * 1.65), member.z());
    ...
    private static float smooth(float value) { float t = Mth.clamp(value, 0.0f, 1.0f); return t * t * (3.0f - 2.0f * t); }
}
