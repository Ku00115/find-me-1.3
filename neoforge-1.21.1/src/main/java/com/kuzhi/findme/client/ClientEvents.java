...
import com.kuzhi.findme.compat.cobblemon.CobblemonClientRidingBridge;
...
public final class ClientEvents {
    ...
    private static void tickKey(CompanionKind kind, KeyMapping key) {
        ...
        if (isDown && !wasDown) {
            ...
            if (kind == CompanionKind.MOUNT) {
                ...
                ClientEvents.sendCobblemon(CobblemonCommandAction.SYNC, -1);
    ...
    private static void sendCobblemon(CobblemonCommandAction action, int slot) {
        if (ModNetwork.channel != null) {
            if (action == CobblemonCommandAction.ACTIVATE && ClientCobblemonState.shouldLockCamera(slot)) {
                ClientCameraLock.arm();
            }
            UUID uuid = ClientCobblemonState.entries().stream()
                    .filter(entry -> entry.slot() == slot)
                    .map(com.kuzhi.findme.network.CobblemonPartyPacket.Entry::uuid)
                    .findFirst().orElse(new UUID(0L, 0L));
    ...
    public static class ForgeEvents {
        ...
        public static void onMouseScroll(InputEvent.MouseScrollingEvent event) {
            ...
            if (ClientFindMeModuleState.enabled(FindMeModule.RIDING)
                ...
                ClientEvents.send(CompanionKind.COMPANION, event.getScrollDeltaY() > 0.0 ? CompanionAction.PREVIOUS : CompanionAction.NEXT, -1);
                event.setCanceled(true);
            }
        }
        ...
        public static void onClientTick(ClientTickEvent.Post event) {
            ...
            SableVehicleScreenshotPreviewCache.tick();
            if (CobblemonCompat.available()
                    && ClientFindMeModuleState.enabled(FindMeModule.COBBLEMON_INTEGRATION)) {
                CobblemonClientRidingBridge.tick();
            }
        ...
        public static void onRenderLevelStage(RenderLevelStageEvent event) {
            ...
            SableVehicleScreenshotPreviewCache.onRenderLevelStage(event);
        }
        ...
        public static void onRenderPlayerPre(RenderPlayerEvent.Pre event) {
            ...
            if (!ClientEntityPreviewRenderGuard.active()
                    && ClientContractCamera.renderLocalRider() && event.getEntity() == minecraft.player
        ...
        public static void onScreenMouseScroll(ScreenEvent.MouseScrolled.Pre event) {
            ...
            if (screen instanceof CompanionWheelScreen wheel) {
                wheel.mouseScrolled(event.getMouseX(), event.getMouseY(), event.getScrollDeltaX(), event.getScrollDeltaY());
                event.setCanceled(true);
            } else if (screen instanceof VehicleWheelScreen wheel) {
                wheel.mouseScrolled(event.getMouseX(), event.getMouseY(), event.getScrollDeltaX(), event.getScrollDeltaY());
                event.setCanceled(true);
            } else if (screen instanceof FindMeDialogueConfigScreen dialogue) {
                dialogue.mouseScrolled(event.getMouseX(), event.getMouseY(), event.getScrollDeltaX(), event.getScrollDeltaY());
                event.setCanceled(true);
            }
        }
        ...
        public static void onRenderGui(RenderGuiEvent.Post event) {
            ...
            ClientRideHomeTransitionState.render(event.getGuiGraphics(), event.getPartialTick().getGameTimeDeltaPartialTick(false));
        }
    }
